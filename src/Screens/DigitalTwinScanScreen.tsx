...
export default function DigitalTwinScanScreen({ onBack, onFinishScan }: DigitalTwinScanScreenProps) {
  ...
  const [seconds, setSeconds] = useState(45.3);
  const [isRecording, setIsRecording] = useState(true);
  const [spatialCoverage, setSpatialCoverage] = useState(87);
  const [keyframes, setKeyframes] = useState(427);
  const [spatialNodes, setSpatialNodes] = useState(1865);
  const [memoryVol, setMemoryVol] = useState(142);
  ...
  const startCamera = async (facing: 'environment' | 'user' = 'environment') => {
    ...
    try {
      ...
      // Attempt 1: Optimal HD with facingMode
      try {
        ...
        console.warn('Attempt 1 (HD + facingMode) failed, trying basic facingMode constraint:', err1);
  ...
  useEffect(() => {
    ...
    // Start camera immediately on mount (works on mobile & webcam)
        ...
        <div className="scan-bottom-controls-wrap">
          ...
          <div className="capture-actions-bar">
            ...
            <button
              ...
              aria-label="Stop and Save Scan"
              title="Stop Recording"
            >
              <div className="shutter-inner-square" />
            </button>
