...
export default function DigitalTwinScanScreen({ onBack, onFinishScan }: DigitalTwinScanScreenProps) {
  ...
  // Sensor States
  const [isAngleCorrect, setIsAngleCorrect] = useState<boolean>(true);
  ...
  // On Mount: Detect Device, Start Camera, and Listen to Sensors
  useEffect(() => {
    ...
    const handleOrientation = (event: DeviceOrientationEvent) => {
      ...
      if (beta !== null) {
        // Ideal vertical angle is ~90 degrees. We'll allow 65 to 115 degrees.
        setIsAngleCorrect(beta > 65 && beta < 115);
      }
    };
    ...
    const handleMotion = (event: DeviceMotionEvent) => {
      ...
      if (acc && acc.x !== null && acc.y !== null && acc.z !== null) {
        // Simple heuristic: Total acceleration variance from 9.8g
        const totalAcc = Math.sqrt(acc.x * acc.x + acc.y * acc.y + acc.z * acc.z);
        setIsSteady(totalAcc > 8.0 && totalAcc < 11.5);
      }
    };
    ...
    <div className="digital-twin-scan-app">
      ...
      <div className="scan-hud-layer">
        ...
        <div className="guidance-banners-stack">
          ...
          <div className={`guidance-banner ${isAngleCorrect ? 'banner-locked' : 'banner-warning'}`}>
            <span className="banner-icon">{isAngleCorrect ? '✓' : '⚠️'}</span>
            <span className="banner-text">
              {isAngleCorrect ? 'Camera angle optimal' : 'Adjust camera angle (keep vertical)'}
            </span>
          </div>
          <div className={`guidance-banner ${isSteady ? 'banner-move' : 'banner-warning'}`}>
            <span className={`banner-icon ${isSteady ? 'spin' : ''}`}>{isSteady ? '🔄' : '🫨'}</span>
            <span className="banner-text">
              {isSteady ? 'Move steadily across room perimeter' : 'Hold camera steady!'}
            </span>
          </div>
        </div>
